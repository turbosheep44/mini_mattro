...
# COPIED (minor modifications) FROM
# https://github.com/cycraig/MP-DQN/blob/master/agents/memory/memory.py
#
# RingBuffer is basically a preallocated memory buffer
# Memory is a collection of ring buffers which stores tuples (state, action, reward, next_state, game_over)
#   and can retrieve them in batches
...
class Memory(object):
    ...
    def append(self, state, action, reward, next_state, terminal, training=True):
        if not training:
            return
